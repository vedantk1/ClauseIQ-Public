...
import { draftKey, emptyPersonal, hasUnconfirmedChanges, sameBrief } from "./workspaceState";
import { Action, BriefForm, fieldClass, markerLabels, Panel, SaveFeedback } from "./WorkspaceControls";
import { DocumentSourceView, EvidenceSourcePane } from "./EvidenceSourcePane";
...
export default function ReviewWorkspace({ documentId }: { documentId: string }) {
  ...
  const [navigationRequest, setNavigationRequest] = useState<{ requestId: number; pageNumber: number }>();
  ...
  const finding = run?.findings.find(item => item.id === selectedFinding) ||
    run?.findings.find(item => item.id === personal.position.finding_id) || run?.findings[0] || null;
  const evidence = finding?.evidence.find(item => item.span_id === selectedEvidence) || null;
  ...
  function navigate(nextView: ReviewPosition["view"], findingId = finding?.id || null, evidenceId = selectedEvidence) {
    setView(nextView);
    setSelectedFinding(findingId);
    setSelectedEvidence(evidenceId);
    if (run) controller!.enqueue({ type: "set_position", run_id: run.id, position: {
      view: nextView, finding_id: findingId, evidence_span_id: evidenceId,
    } });
  }

  function openEvidence(item: ReviewEvidence) {
    setNavigationRequest(previous => ({ requestId: (previous?.requestId || 0) + 1, pageNumber: item.page_number }));
  ...
  function resume() {
    ...
    if (previousEvidence) setNavigationRequest(previous => ({ requestId: (previous?.requestId || 0) + 1, pageNumber: previousEvidence.page_number }));
  ...
  return <div className="mx-auto max-w-[1600px] space-y-4 px-4 py-6 sm:px-6">
    <header className="flex flex-wrap items-start justify-between gap-3">
      <div className="min-w-0">
        ...
        <p className="mt-1 text-sm text-text-secondary">Local source · {extractedPages}/{sourcePages.length || "?"} pages with extractable text · no AI review in this increment</p>
      </div>
    ...
    {run && <div className="rounded-lg border border-accent-amber/50 bg-accent-amber/10 p-4 text-sm">
      <p className="font-semibold">Synthetic example — not an AI-generated review</p>
      <p className="mt-1">Selected example findings for the exact synthetic managed-services PDF, written from Example Customer&apos;s perspective. They are not exhaustive and do not establish legal safety.</p>
      {!sameBrief(run.context, workspace.brief) && <p className="mt-2 font-medium">The saved brief differs from this example&apos;s context. These findings still use their original customer perspective; they have not been rerun.</p>}
      <details className="mt-2"><summary className="cursor-pointer">Example review context</summary><p className="mt-2">{run.context.role || run.context.perspective}</p><p>{run.context.priorities || "No extra priorities supplied."}</p></details>
    </div>}
    ...
    <nav aria-label="Agreement workspace views" className="flex flex-wrap gap-2 border-b border-border-muted pb-3">
      ...
      {workspace.runs.length > 1 && <label className="ml-auto text-sm">Review run <select className="ml-2 rounded border border-border-muted bg-bg-surface p-2" value={run?.id || ""} onChange={event => {
        setSelectedRun(event.target.value); setSelectedFinding(null); setSelectedEvidence(null); setView("overview");
      }}>{workspace.runs.map((item, index) => <option key={item.id} value={item.id}>Synthetic example {index + 1}</option>)}</select></label>}
    </nav>
    ...
    {view === "overview" && <div className="grid items-start gap-4 lg:grid-cols-[1.2fr_1fr]">
      <div className="space-y-4">
        ...
        <Panel>
          <h2 className="text-lg font-semibold">{run ? "Example agreement overview" : "Source saved. Review not started."}</h2>
          <p className="mt-3 whitespace-pre-wrap text-sm leading-relaxed">{run?.overview || "You can inspect the original and save a review brief without a key. New AI review generation is not available in this implementation yet."}</p>
          {run ? <Action className="mt-4" onClick={() => navigate("findings")}>Explore {run.findings.length} example findings</Action> : <>
            <Action className="mt-4" disabled>AI review — coming in the next increment</Action>
            {workspace.fixture_available && <div className="mt-4 border-t border-border-muted pt-4"><p className="mb-3 text-sm text-text-secondary">This source matches the supported synthetic fixture. Load the labelled customer-perspective example to exercise saved work. Your brief is not used to generate it.</p><Action disabled={blocked || state.pending > 0} onClick={() => controller.createFixture()}>Load synthetic customer-perspective example</Action></div>}
          </>}
        </Panel>
        <Panel>
    ...
    {view === "findings" && (run && finding ? <div className="grid items-start gap-4 xl:grid-cols-[230px_minmax(0,1fr)_minmax(0,1fr)]">
      <Panel><h2 className="font-semibold">Example findings</h2><div className="mt-3 space-y-2">{run.findings.map(item => <button key={item.id} onClick={() => openFinding(item.id)} aria-current={finding.id === item.id ? "true" : undefined}
      ...
      <div className="space-y-4">
        <Panel>
          <p className="text-xs uppercase tracking-wider text-text-secondary">Example finding</p><h2 className="mt-1 text-xl font-semibold">{finding.title}</h2>
          <div className="mt-4 space-y-4 text-sm leading-relaxed">
            <div><h3 className="font-semibold">What the agreement says</h3><p className="mt-1">{finding.facts}</p></div>
            <div><h3 className="font-semibold">Why this matters for the example reviewer</h3><p className="mt-1">{finding.interpretation}</p></div>
        ...
        <Panel>
          ...
          <details className="mt-4 text-sm"><summary className="cursor-pointer">Authored example suggestion</summary><p className="mt-2">{finding.suggested_question}</p></details>
        </Panel>
    ...
    </div> : <Panel><h2 className="text-lg font-semibold">No review run yet</h2><p className="mt-2 text-sm">An empty finding list means review has not run—not that the agreement has no issues.</p><Action className="mt-3" onClick={() => navigate("overview")}>Return to overview</Action></Panel>)}

    {view === "document" && <DocumentSourceView documentId={documentId} filename={filename} source={source} finding={finding} evidence={evidence}
      navigationRequest={navigationRequest} onReturn={() => navigate("findings")} />}
